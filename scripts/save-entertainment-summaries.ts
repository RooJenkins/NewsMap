...
// ALL COUNTRIES with lat/lng data (128 countries)
const ALL_COUNTRIES = [
  ...
  { name: 'Samoa', type: 'country', lat: -13.7590, lng: -172.1046, country: 'Samoa' },
  { name: 'Tonga', type: 'country', lat: -21.1789, lng: -175.1982, country: 'Tonga' },
...
export async function saveEntertainmentSummaries(entries: EntertainmentEntry[]) {
  console.log(`\n🎬 Saving ${entries.length} entertainment summaries to database...`)
  ...
  for (const entry of entries) {
    // Find country data
    const countryData = ALL_COUNTRIES.find(c => c.name === entry.name)

    if (!countryData) {
      console.error(`❌ Country data not found for: ${entry.name}`)
      errorCount++
      errors.push(`Country data not found: ${entry.name}`)
      continue
    }

    try {
      console.log(`\n📝 Saving ${entry.name}...`)
      console.log(`   Length: ${entry.summary.length} characters`)
  ...
  console.log(`❌ Errors: ${errorCount}/${entries.length}`)
...
async function main() {
  // This script is imported by other scripts that provide the data
  console.log('This script should be imported, not run directly')
}
