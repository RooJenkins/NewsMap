...
async function saveWriteup(country: typeof ALL_COUNTRIES[0], data: WriteupData) {
  try {
    ...
    console.log(`✅ ${country.name} - Saved (${data.summary.length} chars, ${data.issues.length} issues)`)
    return true
  } catch (error) {
...
// Main function - takes writeup data via stdin
async function main() {
  const fs = require('fs')

  console.log('📥 Reading writeup data from stdin...')
  const input = fs.readFileSync(0, 'utf-8')
  const writeups: Record<string, WriteupData> = JSON.parse(input)

  console.log(`📊 Processing ${Object.keys(writeups).length} writeups\n`)
  ...
  for (const country of ALL_COUNTRIES) {
    if (writeups[country.name]) {
      const result = await saveWriteup(country, writeups[country.name])
      if (result) saved++
      else skipped++
    }
  }
