...
// Ezra Klein voice prompt
function generatePrompt(country: string): string {
  return `You are writing in the distinctive voice and style of Ezra Klein.
...
**Language Patterns:** Clear, exploratory, "I've been thinking about..."

**Avoid:** Hot takes, partisan point-scoring

---

Generate a comprehensive summary of **Lifestyle** in **${country}** based on the latest November 2025 news.

Use web search or your knowledge to find real, current events related to lifestyle in ${country}.

**Format:**

# ${country} - Lifestyle

[Opening hook in Ezra Klein's distinctive style - ~150 words]

## The Big Issue 1: [Title]
[Deep dive ~200 words in Ezra Klein's voice]
...
[~150 words on global implications in Ezra Klein's style]

## The Bottom Line
[~100 words synthesis and forward look in Ezra Klein's voice]

**CRITICAL REQUIREMENTS:**
- Write EXACTLY as Ezra Klein would write
...
async function generateLifestyleSummary(
  country: string,
  anthropic: Anthropic
...
async function generateAllLifestyle() {
  ...
  console.log('📝 GENERATING LIFESTYLE WRITEUPS - ALL 114 COUNTRIES')
  console.log('🎤 Voice: Ezra Klein')
  console.log('📊 Total: 114 countries\n')
  ...
  for (let i = 0; i < ALL_COUNTRIES.length; i++) {
    ...
    try {
      ...
      if (existing) {
        ...
        if (countryNum % 25 === 0) {
          console.log(`\n${'='.repeat(60)}`)
          console.log(`📊 PROGRESS REPORT - ${countryNum}/114 countries processed`)
      ...
      console.log(`  🤖 Generating with Ezra Klein voice...`)

      const { summary, issues } = await generateLifestyleSummary(
        country.name,
        anthropic
      )
      ...
      // Report every 25 even on errors
      if (countryNum % 25 === 0) {
        console.log(`\n${'='.repeat(60)}`)
        console.log(`📊 PROGRESS REPORT - ${countryNum}/114 countries processed`)
        console.log(`   ✅ Generated: ${generated}`)
  ...
  console.log(`   ⏭️  Skipped: ${skipped}`)
  console.log(`   ❌ Errors: ${errors}`)
  console.log(`   📊 Total: ${generated + skipped + errors}/114`)
  console.log(`${'='.repeat(60)}\n`)
}

generateAllLifestyle()
  .catch(console.error)
