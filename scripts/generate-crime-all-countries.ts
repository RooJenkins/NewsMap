...
function generatePrompt(country: string): string {
  return `You are writing in the distinctive voice and style of ${CRIME_CATEGORY.voiceLegend}.

**Voice Characteristics:**
${CRIME_CATEGORY.voiceCharacteristics.map(c => `- ${c}`).join('\n')}

**Opening Style:** ${CRIME_CATEGORY.openingStyle}

**Language Patterns:** ${CRIME_CATEGORY.languagePatterns}

**Avoid:** ${CRIME_CATEGORY.avoid}

---

Generate a comprehensive summary of **${CRIME_CATEGORY.name}** in **${country}** based on the latest November 2025 news.

Use web search or your knowledge to find real, current events related to ${CRIME_CATEGORY.name.toLowerCase()} in ${country}.

**Format:**

# ${country} - ${CRIME_CATEGORY.name}

[Opening hook in ${CRIME_CATEGORY.voiceLegend}'s distinctive style - ~150 words]

## The Big Issue 1: [Title]
[Deep dive ~200 words in ${CRIME_CATEGORY.voiceLegend}'s voice]
...
[~150 words on global implications in ${CRIME_CATEGORY.voiceLegend}'s style]

## The Bottom Line
[~100 words synthesis and forward look in ${CRIME_CATEGORY.voiceLegend}'s voice]

**CRITICAL REQUIREMENTS:**
- Write EXACTLY as ${CRIME_CATEGORY.voiceLegend} would write
...
async function generateCategorySummary(
  ...
  try {
    ...
    console.error(`Error generating ${CRIME_CATEGORY.name} summary for ${country}:`, error)
    throw error
  }
}

// Main generation function
async function generateCrimeWriteups() {
  ...
  console.log('========================================')
  console.log('Crime & Justice Writeups Generation')
  ...
  for (let i = 0; i < ALL_COUNTRIES.length; i++) {
    ...
    try {
      ...
      if (existing) {
        ...
        console.log(`  Generating with ${CRIME_CATEGORY.voiceLegend} voice...`)
